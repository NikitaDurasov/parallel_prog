...
vector<Walker> merge(vector<Walker> a, vector<Walker> b)
{
	int n = a.size();
    int m = b.size();
    vector<Walker> c(n + m);
...
void initialize_walkers(int num_walkers_per_proc, int max_walk_size,
  ...
  for (int i = 0; i < num_walkers_per_proc; i++) {
    ...
    walker.num_steps_left_in_walk = rand() % max_walk_size;
...
void receive_incoming_walkers(vector<Walker>* incoming_walkers,
  ...
  vector<Walker> buff;
  buff.resize(incoming_walkers_size / sizeof(Walker));
  MPI_Recv((void*)buff.data(), incoming_walkers_size,
           MPI_BYTE, incoming_rank, 0, MPI_COMM_WORLD,
           MPI_STATUS_IGNORE);
  *incoming_walkers = merge(buff, *incoming_walkers);
}
